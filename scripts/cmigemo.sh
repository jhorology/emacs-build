...
function cmigemo_build ()
{
    ...
    # ${mingw_prefix}-iconv provides iconv.exe in ${MINGW_PREFIX}/bin/,
    # which cmake's find_program can locate via the MinGW PATH.
    # (${mingw_prefix}-libiconv only provides the DLL, not the executable.)
    ensure_packages cmake perl curl gzip ${mingw_prefix}-iconv
    ...
    # Place migemo-dict under share/migemo/ for Emacs packages that look there
